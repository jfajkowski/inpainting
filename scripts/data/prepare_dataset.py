...
parser.add_argument('--frames-dir', type=str, default='data/interim/demo/Images')
...
for b in background:
    for f in foreground:
        if (opt.mode == 'cross' and b != f[0]) or (opt.mode == 'match' and b == f[0]):
...
for background_index, (foreground_index, foreground_object_stats_row) in tqdm(candidates,
    ...
    save_frames(frames, f'{opt.processed_dir}/{opt.frame_type.capitalize()}s/{sequence_name}', opt.frame_type)
