...
parser.add_argument('--model-dir', type=str, default='../models/03022020_base_wgan')
...
[generator, discriminator], [generator_optimizer, discriminator_optimizer] = amp.initialize(
    [generator, discriminator], [generator_optimizer, discriminator_optimizer], opt_level=opt.opt_level)

writer = SummaryWriter(opt.model_dir)

checkpoint_paths = glob.glob(opt.model_dir + '/*.pth')
if checkpoint_paths:
    checkpoint = torch.load(sorted(checkpoint_paths)[-1], map_location=lambda storage, loc: storage.cuda(0))
...
for epoch in range(start_epoch, opt.epochs + 1):
    ...
    for step in range(1, opt.train_steps + 1):
        ...
        if step % opt.n_critic == 0:
            ...
            adversarial_loss = -torch.mean(discriminator(image_filled))
        ...
        loss = -torch.mean(discriminator(image)) + torch.mean(discriminator(image_filled.detach()))
    ...
    with torch.no_grad():
        for step in range(1, opt.val_steps + 1):
            ...
            loss = -torch.mean(discriminator(image)) + torch.mean(discriminator(image_filled))
