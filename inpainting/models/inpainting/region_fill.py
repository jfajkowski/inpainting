...
def formRightSide(I, maskPerimeter):
    ...
    rightSide[1:height - 1, 1:width - 1] = perimeterValues[0:height - 2, 1:width - 1] + \
    ...
    rightSide[1:height - 1, width - 1] = perimeterValues[0:height - 2, width - 1] + \
    ...
    rightSide[height - 1, 1:width - 1] = perimeterValues[height - 2, 1:width - 1] + \
                                         perimeterValues[height - 1, 0:width - 2] + \
                                         perimeterValues[height - 1, 2:width]

    rightSide[0, 0] = perimeterValues[0, 1] + \
                      perimeterValues[1, 0]
...
def inpaint(image, mask):
    ...
    image = (image * (1 - mask)).cpu().numpy()
    mask = mask.cpu().numpy()

    result = np.zeros_like(image)
    for i in range(b):
        for j in range(c):
            result[i, j, :, :] = regionfill(image[i, j, :, :], mask[i, 0, :, :])
