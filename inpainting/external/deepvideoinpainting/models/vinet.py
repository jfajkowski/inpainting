from inpainting.external.deepvideoinpainting.models.ConvLSTM import ConvLSTM
...
class VI_2D_Decoder_3(nn.Module):
    def __init__(self, opt):
        ...
        #### UPCONV
        self.dc1_1 = GatedUpConvolution((1, opt.crop_size // 4 // dv, opt.crop_size // 4 // dv), 128, 96,
                                        kernel_size=(1, 3, 3), stride=(1, 1, 1), padding=(0, 1, 1), bias=False)

        self.dc2_1 = GatedConvolution(96 + 96, 96, kernel_size=(1, 3, 3), stride=(1, 1, 1), padding=(0, 1, 1),
                                      bias=False)
        self.dc2_bt1 = GatedConvolution(96, 96, kernel_size=(1, 3, 3), stride=(1, 1, 1), dilation=(1, 2, 2),
                                        padding=(0, 2, 2), bias=False)
        self.dc2_bt2 = GatedConvolution(96, 96, kernel_size=(1, 3, 3), stride=(1, 1, 1), dilation=(1, 4, 4),
                                        padding=(0, 4, 4), bias=False)
        self.dc2_bt3 = GatedConvolution(96, 96, kernel_size=(1, 3, 3), stride=(1, 1, 1), dilation=(1, 8, 8),
                                        padding=(0, 8, 8), bias=False)
        #### UPCONV
        self.dc2_2 = GatedUpConvolution((1, opt.crop_size // 2 // dv, opt.crop_size // 2 // dv), 96, 64,
                                        kernel_size=(1, 3, 3), stride=(1, 1, 1), padding=(0, 1, 1), bias=False)
        self.dc3_1 = GatedConvolution(64 + 64, 64, kernel_size=(1, 3, 3), stride=(1, 1, 1), padding=(0, 1, 1),
                                      bias=False)
        self.dc3_2 = GatedConvolution(64, 64, kernel_size=(1, 3, 3), stride=(1, 1, 1), padding=(0, 1, 1), bias=False)
        #### UPCONV
        self.dc4 = GatedUpConvolution((1, opt.crop_size // dv, opt.crop_size // dv), 64, 32, kernel_size=(1, 3, 3),
                                      stride=(1, 1, 1), padding=(0, 1, 1), bias=False)
        if self.opt.double_size:
            self.upsample = nn.Upsample(size=(1, opt.crop_size, opt.crop_size), mode='trilinear', align_corners=True)
    ...
    def forward(self, x, x2_64_warp=None, x2_128_warp=None):
        ...
        if x2_64_warp is not None and x2_128_warp is not None:
            ...
            if self.opt.double_size:
                d6 = self.dc6(
                    self.dc5(self.upsample(self.dc4(self.dc3_2(self.dc3_1(torch.cat([x1_128, x2_128_warp], 1)))))))
            else:
...
class VI_2D_BottleNeck(nn.Module):
    def __init__(self, opt, in_ch):
        ...
        self.opt = opt
        ### bottleneck
        self.bt0 = GatedConvolution(in_ch, 128, kernel_size=(3, 3), stride=(1, 1), dilation=(1, 1), padding=(1, 1),
                                    bias=False, type='2d')
        self.bt1 = GatedConvolution(128, 128, kernel_size=(3, 3), stride=(1, 1), dilation=(2, 2), padding=(2, 2),
                                    bias=False, type='2d')
        self.bt2 = GatedConvolution(128, 128, kernel_size=(3, 3), stride=(1, 1), dilation=(4, 4), padding=(4, 4),
                                    bias=False, type='2d')
        self.bt3 = GatedConvolution(128, 128, kernel_size=(3, 3), stride=(1, 1), dilation=(8, 8), padding=(8, 8),
                                    bias=False, type='2d')
...
class VI_Aggregator(nn.Module):
    def __init__(self, opt, in_ch, T):
        ...
        self.stAgg = GatedConvolution(in_ch, in_ch, kernel_size=(T, 3, 3), stride=(1, 1, 1), padding=(0, 1, 1),
                                      bias=False, type='3d')
...
class VINet_final(nn.Module):
    ...
    def forward(self, masked_img, mask, prev_state=None, prev_feed=None, idx=0):  # masked img: b x C x TxHxW
        ...
        # ============================ SCALE - 1/4 : 64 =============================
        flow2_64 = F.upsample(flow2, scale_factor=2, mode='bilinear', align_corners=True) * 2
        flow3_64 = F.upsample(flow3, scale_factor=2, mode='bilinear', align_corners=True) * 2
        flow4_64 = F.upsample(flow4, scale_factor=2, mode='bilinear', align_corners=True) * 2
        flow5_64 = F.upsample(flow5, scale_factor=2, mode='bilinear', align_corners=True) * 2
        flow6_64 = F.upsample(flow6, scale_factor=2, mode='bilinear', align_corners=True) * 2
        ...
        # ============================= SCALE - 1/2 : 128 ===============================
        flow2_128 = F.upsample(flow2_64, scale_factor=2, mode='bilinear', align_corners=True) * 2
        flow3_128 = F.upsample(flow3_64, scale_factor=2, mode='bilinear', align_corners=True) * 2
        flow4_128 = F.upsample(flow4_64, scale_factor=2, mode='bilinear', align_corners=True) * 2
        flow5_128 = F.upsample(flow5_64, scale_factor=2, mode='bilinear', align_corners=True) * 2
        flow6_128 = F.upsample(flow6_64, scale_factor=2, mode='bilinear', align_corners=True) * 2
        ...
        output, _ = self.decoder(state[0].unsqueeze(2), x2_64_warp=f_syn_64.unsqueeze(2),
                                 x2_128_warp=f_syn_128.unsqueeze(2))
        occ_mask = F.upsample(occlusion_mask, scale_factor=8, mode='bilinear', align_corners=True)
        occ_mask_64 = F.upsample(occlusion_mask_64, scale_factor=4, mode='bilinear', align_corners=True)
        occ_mask_128 = F.upsample(occlusion_mask_128, scale_factor=2, mode='bilinear', align_corners=True)

        flow6_256, flow6_512 = None, None
        if self.opt.prev_warp:
            if prev_state is not None or idx != 0:
                flow6_256 = F.upsample(flow6_128, scale_factor=2, mode='bilinear', align_corners=True) * 2
                flow6_512 = F.upsample(flow6_128, scale_factor=4, mode='bilinear', align_corners=True) * 4
                ...
                if self.opt.double_size:
                    ...
                    output = (1 - occlusion_mask_512.unsqueeze(
                        2)) * output + occlusion_mask_512 * prev_feed_warp.unsqueeze(2)
                    flow6_256 = flow6_512
                else:
                    prev_feed_warp = self.warping_256(prev_feed[:, :3], flow6_256)
                    output = (1 - occlusion_mask_256.unsqueeze(
                        2)) * output + occlusion_mask_256 * prev_feed_warp.unsqueeze(2)
                if self.opt.loss_on_raw:
                    output = (output, output_)

        return output, torch.stack([flow2_128, flow3_128, flow4_128, flow5_128, flow6_128], 2), state, torch.stack(
            [occ_mask, 1 - occ_mask, occ_mask_64, 1 - occ_mask_64, occ_mask_128, 1 - occ_mask_128], 2), flow6_256
