...
class MaskEstimator_(nn.Module):
    def __init__(self, args, ch_in):
        ...
        self.convs = nn.Sequential(
            ...
            nn.Conv2d(in_channels=ch_in // 2, out_channels=1, kernel_size=3, stride=1, padding=1, dilation=1, groups=1,
                      bias=True),
            nn.Sigmoid()
        )
...
class LongFlowEstimatorCorr(nn.Module):
    def __init__(self, args, ch_in):
        ...
        self.conv1 = nn.Conv2d(in_channels=32, out_channels=2, kernel_size=3, stride=1, padding=1, dilation=1, groups=1,
                               bias=True)
...
class LongFlowNetCorr(nn.Module):
    def __init__(self, args, in_ch):
        ...
        self.corr = Correlation(pad_size=args.search_range, kernel_size=1, max_displacement=args.search_range,
                                stride1=1, stride2=1, corr_multiply=1).cuda()
